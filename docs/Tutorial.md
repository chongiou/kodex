...
Kodex 是一个在 `zdjl` 环境中构建对话框的渲染引擎。它使用开发者熟悉的 JSX 语法来声明式地描述 UI，并内置了基于**信号 (Signal)** 的响应式系统，让数据驱动视图更新变得简单直观。
...
## **FAQ 补充**
- **Q**：为什么有些组件既有小写又有大写形式？  
  **A**：小写是基础实现，大写是您编写的自定义组件或其他开发者封装的自定义组件。  
- **Q**：自定义组件能否用小写？  
  **A**：不能，构建工具将小写开头的标签转为字符串，大写开头则转为变量引用，kodex 靠此识别。
- **Q**：kodex 看起来像个转换器或转译器, 凭什么说'渲染'? 渲染不应该是更底层的行为吗?   
  **A**：转换器的工作通常是静态的、一次性的。渲染引擎则是一个动态的、持续的过程。  
    > kodex 的工作流程远超简单转换。其在运行时动态地管理 UI 的整个生命周期，包括用户交互、数据更新和最终的销毁清理。这个持续、动态的过程远比转换器的静态、一次性任务复杂得多。  
    > 无论是 Kodex、Vue 还是 React，它们的 “渲染” 都不直接控制屏幕上的单个像素。而指的是将声明式的组件代码转化为宿主平台上最终呈现和持续更新的 UI 的整个过程。
...
```jsx
function App() {
  return jsx(Fragment, /* 在 JSX 里注释*/{ children: [
    jsx(
      "text",
      {
        // 在 JSX 注释
...
现在您已经了解 `Kodex` 是一个“运行时”引擎，那么向组件传递信号的规则就变得非常清晰了：**您传递给属性的，必须是能让 Kodex 在运行时识别出其为“响应式”的线索。** 这是和 `SolidJS` 不同的部分。
...
- **声明式**：你的 `JSX` 现在只描述 "显示什么" ，而不是 "如何计算" 。   
`hidden={isCountTooLow}` 完美地表达了意图：这个元素的隐藏状态由 `isCountTooLow` 决定。
- **高效**：如果 `isCountTooLow` 被多个组件使用，很容易可以应用缓存。  
使用 `createMemo`， 让它的计算结果（`true` 或 `false`）只会在 `count` 变化时计算一次，然后通知所有依赖它的地方，避免重复计算。
- **可测试与复用**：`isCountTooLow` 和 `buttonColor` 是纯粹的 JavaScript 函数，你可以轻松地将它们提取到单独的文件中，进行单元测试，并在应用的其他地方复用。

### 优化复杂计算

当你的衍生逻辑非常昂贵（例如，需要过滤和排序一个大数组, 每次计算需要的时间比较长），或者你就是想明确地创建一个可缓存的衍生信号时，可以使用 `createMemo`。

`createMemo` 接收一个函数，并返回一个**只读**的、记忆化的信号。它会追踪其内部的依赖，仅在依赖变化时才重新计算。

```jsx
import { createSignal, createMemo } from '@zdjl/kodex'
...
// 使用 createMemo 创建一个昂贵的衍生信号
const activeUserNames = createMemo(() => {
...
3. 如果这个计算非常昂贵或被多处复用，使用 `createMemo` 来优化性能。
...
如果您的目标是根据信号计算一个新值并显示在 UI 中，请使用衍生信号 (`() => ...`) 或` createMemo`。
...
## 使用增强组件
增强组件是 kodex 基于原生组件封装的自定义组件，这一部分不包含在核心包中

### 条件显示

⚠️ **重要**: Kodex **无法**在运行时动态地添加或删除UI元素（条件渲染）。目标环境的限制决定了UI结构在渲染时必须是固定的。  
✅ 为了减少沟通成本，我们称之为**条件显示**——根据条件切换元素的可见性。
...
有时候我们真的不得不更改 UI 结构以完成需求，但目标环境所限制无法完成局部结构更新。  
所以 kodex 的动态渲染是通过重新构造 UI 结构并重新弹窗来实现的。  
> 虽然效果并不感人（重新弹窗）但也完成了功能。
